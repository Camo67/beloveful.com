...
interface TravelPortfolioDropdownProps {
  variant?: "white" | "auto";
}

export function TravelPortfolioDropdown({ variant = "auto" }: TravelPortfolioDropdownProps) {
  const [isOpen, setIsOpen] = useState(false);
  const albums = getAllAlbumsSorted();

  // Desired region display order (exclude "Erasing Borders" from list)
  ...
  // Group albums by region (excluding Erasing Borders) and sort countries within each region
  const albumsByRegion = useMemo(() => {
    const filtered = albums.filter(album => album.region !== "Erasing Borders" && album.region !== "Logo");
